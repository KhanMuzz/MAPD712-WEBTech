...
//------------------- THIS SCREEN ALLOWS USER TO PERFORM SEARCH BY DOCTOR -------------------------


class SearchPatientByDoctor extends React.Component { 
    ...
    //Function to handle Api call to MondoDB running on local host server 
      ...
      //Add Dr to users input to match Database
      input = "Dr. "+ charArray.join("")
      ...
      fetch(uri,
        {
          ...
          { 
            ...
            if(respJson.length == 0){
              alert("Sorry, no records for that doctor name")
            }
          }).catch((error) => {
            console.error(error)}
            )
            ...
            <View style={{flex: 1}}>
              <View style={{flex: 1, flexDirection: 'row', justifyContent:'center', marginTop: 5, alignItems:'center'}}>
                  <View style={{width: 160, height: 53, justifyContent:'center', alignItems:'center'}}>
                    <Text style={{fontSize:18, fontWeight:'bold'}}>Doctor's Name:</Text>
                  </View> 
...
export default SearchPatientByDoctor;
